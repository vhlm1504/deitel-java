...
Embora métodos não-private static são herdados, eles não podem ser sobrescritos. Então, já que métodos abstratos são feitos para serem sobrescritos para que processem objetos do seu próprio tipo, não faz sentido declarar static métodos abstrated.
